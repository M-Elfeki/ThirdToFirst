...
data_dir = '/home/elfeki/Workspace/Generation_Datasets/Real_Dataset/Optical_Flow_lk/'+presp+'/'
output_path = '/home/elfeki/Workspace/Generation_Datasets/Real_Dataset/OF/'+presp+'/'
...
counter = 0
flow_len = 30
gaussian_weights = gaussian_filter1d(np.float_([0]*(flow_len/2)+[1]+[0]*((flow_len/2)-1)), 1)
...
def compress_annotations(ann):
    ...
    for i in range(1, len(ann)):
        if cur_action != ann[i]:
            result.append(cur_action + '_' + str(cur_action_start*4*2) + '_' + str(i*4*2))
            cur_action = ann[i]
            cur_action_start = i
    result.append(cur_action + '_' + str(cur_action_start*4*2) + '_' + str(len(ann)*4*2))
    return result
    ...
    for cur_camera_idx in range(2):
        ...
        for cur_action_range in annotations:
            ...
            if cur_action.lower() != action_type.lower():
                continue
            opt_x, opt_y = [], []
            for cur_frame_idx in range(start, end, 2):
            ...
            for cur_frame_idx in range((flow_len / 2), len(opt_x) - (flow_len / 2)):
                ...
                cur_sample[cur_camera_idx].append(np.array([avg_x, avg_y]))
                if cur_camera_idx == 1:
...
def write_list_to_folder(cam_1, cam_2, set_num, action_name, frame_num, folder_dir):
    ...
    for k in range(len(cam_1)):
        sio.savemat(str(folder_dir + 'Camera_1/' + str(counter) + '_' + str(set_num[k]) + '_' + action_name[k] + '_' + str(frame_num[k]) + '.mat'), mdict={'opt_flow': cam_1[k]})
        sio.savemat(str(folder_dir + 'Camera_2/' + str(counter) + '_' + str(set_num[k]) + '_' + action_name[k] + '_' + str(frame_num[k]) + '.mat'), mdict={'opt_flow': cam_2[k]})
        print counter,
        counter += 1
...
def generate_Testing_Data():
    ...
    h5f = h5py.File(output_path+'Real_'+data_dir.split('/')[-1]+'_TestingData.h5', 'w')
