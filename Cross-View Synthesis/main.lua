...
opt = {
   DATA_ROOT = '/home/user/Documents/Elfeki/ReConstruction',         -- path to images (should have subfolders 'train', 'val', etc)
   batchSize = 1,          -- # images in batch
   loadSize = 227,         -- scale images to this size
   fineSize = 227,         --  then crop to this size
   ngf = 64,               -- #  of gen filters in first conv layer
   ndf = 64,               -- #  of discrim filters in first conv layer
   input_nc = 2,           -- #  of input image channels
   output_nc = 2,          -- #  of output image channels
   niter = 35,            -- #  of iter at starting learning rate  -- was 200
   lr = 0.0002,            -- initial learning rate for adam
   beta1 = 0.5,            -- momentum term of adam
   beta2 = 0.999,		   -- 2nd parameter for adam
   ntrain = math.huge,     -- #  of examples per epoch. math.huge for full dataset
   flip = 0,               -- if flip the images for data argumentation
   display = 0,            -- display samples while training. 0 = false
   display_id = 10,        -- display window id.
   gpu = 0,                -- gpu = 0 is CPU mode. gpu=X is GPU mode on GPU X, gpu = 1 used generally
   name = 'optical_flow',              -- name of the experiment, should generally be passed on the command line
   ...
   save_latest_freq = 10000,     -- save the latest model every latest_freq sgd iterations (overwrites the previous latest model)
   ...
   which_model_netG = 'encoder_decoder',  -- selects model to use for netG
   n_layers_D = 0,             -- only used if which_model_netD=='n_layers'
   lambda = 100,               -- weight on L1 term in objective
   -- scale_factor = 4 ,            --scale between input and output dimensions
...
function defineG(input_nc, output_nc, ngf)
    local netG = nil
    if     opt.which_model_netG == "encoder_decoder" then netG = define_ED_opticalflow(input_nc, output_nc, ngf)
...
function defineD(input_nc, output_nc, ndf)
    ...
    if     opt.which_model_netD == "basic" then netD = defineD_opticalflow(input_nc_tmp, output_nc, ndf)
...
local fDx = function(x)
    ...
    -- Real
    local output = torch.FloatTensor(real_AB:size()):fill(fake_label)
    output = netD:forward(real_AB)
...
for epoch = 1 + opt.which_epoch, opt.niter do
    ...
    for i = 1, math.min(data:size(), opt.ntrain), opt.batchSize do
        ...
        if opt.use_GAN==1 then optim.adam(fDx, parametersD, optimStateD) optim.adam(fDx, parametersD, optimStateD) end
