...
function define_ED_opticalflow(input_nc, output_nc, ngf)

    local netG = nn.Sequential()

    netG:add(nn.SpatialConvolution(input_nc, ngf, 3, 3, 2, 2, 1, 1))
    netG:add(nn.LeakyReLU(0.2, true))

    netG:add(nn.SpatialConvolution(ngf, ngf * 2, 4, 4, 2, 2, 0, 0))
    netG:add(nn.SpatialBatchNormalization(ngf * 2)):add(nn.LeakyReLU(0.2, true))

    netG:add(nn.SpatialConvolution(ngf * 2, ngf * 4, 4, 4, 2, 2, 1, 1))
    netG:add(nn.SpatialBatchNormalization(ngf * 4)):add(nn.LeakyReLU(0.2, true))

    netG:add(nn.SpatialConvolution(ngf * 4, ngf * 8, 4, 4, 2, 2, 1, 1))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.LeakyReLU(0.2, true))

    netG:add(nn.SpatialConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 0, 0))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.LeakyReLU(0.2, true))

    netG:add(nn.SpatialConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.LeakyReLU(0.2, true))

    netG:add(nn.SpatialConvolution(ngf * 8, ngf * 8, 3,  3, 1, 1, 0, 0))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.LeakyReLU(0.2, true))




    netG:add(nn.SpatialFullConvolution(ngf * 8, ngf * 8, 3,  3, 1, 1, 0, 0))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.ReLU(true))

    netG:add(nn.SpatialFullConvolution(ngf * 8, ngf * 8, 4,  4, 2, 2, 1, 1))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.ReLU(true))

    netG:add(nn.SpatialFullConvolution(ngf * 8, ngf * 8, 4,  4, 2, 2, 0, 0))
    netG:add(nn.SpatialBatchNormalization(ngf * 8)):add(nn.ReLU(true))
    ...
    netG:add(nn.SpatialFullConvolution(ngf * 2, ngf, 4,  4, 2, 2, 0, 0))
    netG:add(nn.SpatialBatchNormalization(ngf)):add(nn.ReLU(true))

    netG:add(nn.SpatialFullConvolution(ngf, output_nc, 3,  3, 2, 2, 1, 1))
    netG:add(nn.Tanh())
...
function defineG_encoder_decoder(input_nc, output_nc, ngf)
    ...
    -- input is (ngf * 8) x 8 x 8
    local d4 = e5 - nn.ReLU(true) - nn.SpatialFullConvolution(ngf * 8, ngf * 8, 4, 4, 2, 2, 1, 1) - nn.SpatialBatchNormalization(ngf * 8)
    -- input is (ngf * 8) x 16 x 16
...
function defineD_3channel_64(input_nc, output_nc, ndf)

    local netD = nn.Sequential()
    netD:add(nn.SpatialConvolution(input_nc+output_nc, ndf, 4, 4, 2, 2, 1, 1))
    netD:add(nn.LeakyReLU(0.2, true))

    netD:add(nn.SpatialConvolution(ndf, ndf * 4, 4, 4, 2, 2, 1, 1))
...
function defineD_opticalflow(input_nc, output_nc, ndf)

	local netD = nn.Sequential()

	-- input is (nc) x 227 x 227
	netD:add(nn.SpatialConvolution(input_nc+output_nc, ndf, 3, 3, 2, 2, 1, 1))
    netD:add(nn.LeakyReLU(0.2, true))

    -- input is (ndf) x 114 x 114
    netD:add(nn.SpatialConvolution(ndf, ndf * 2, 4, 4, 2, 2, 0, 0))
    netD:add(nn.SpatialBatchNormalization(ndf * 2)):add(nn.LeakyReLU(0.2, true))

    -- input is (ndf*2) x 56 x 56
    netD:add(nn.SpatialConvolution(ndf * 2, ndf * 4, 4, 4, 2, 2, 1, 1))
    netD:add(nn.SpatialBatchNormalization(ndf * 4)):add(nn.LeakyReLU(0.2, true))

    -- input is (ndf*4) x 28 x 28
    netD:add(nn.SpatialConvolution(ndf * 4, ndf * 8, 4, 4, 1, 1, 1, 1))
    netD:add(nn.SpatialBatchNormalization(ndf * 8)):add(nn.LeakyReLU(0.2, true))

    -- input is (ndf*8) x 27 x 27
    netD:add(nn.SpatialConvolution(ndf * 8, 1, 4, 4, 1, 1, 1, 1))
    -- output is 1 x 26 x 26

    -- netD:add(nn.SpatialConvolution(ndf * 8, ndf * 4, 4, 4, 2, 2, 0, 0))
    -- netD:add(nn.SpatialBatchNormalization(ndf * 4)):add(nn.LeakyReLU(0.2, true))

    -- netD:add(nn.SpatialConvolution(ndf * 4, ndf * 2, 4, 4, 2, 2, 1, 1))
    -- netD:add(nn.SpatialBatchNormalization(ndf * 2)):add(nn.LeakyReLU(0.2, true))

    -- netD:add(nn.SpatialConvolution(ndf * 8, 1, 3,  3, 1, 1, 0, 0))
    netD:add(nn.Sigmoid())
